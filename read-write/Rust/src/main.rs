...
fn main() {
    match copy_file_contents(
        "/workspaces/Rusty-Kernels-Code/read-write/source.txt",
        "/workspaces/Rusty-Kernels-Code/read-write/destination.txt",
    ) {
        Ok(_) => (),
...
fn copy_file_contents(source_path: &str, dest_path: &str) -> Result<(), Box<dyn Error>> {
    ...
    let mut dest_file = match File::create(dest_path) {
        ...
        Err(_) => {
            return Err(
                format!("Failed to create or open destination file '{}'.", dest_path).into(),
            )
        }
    };
    ...
    if let Err(_) = source_file.read_to_string(&mut buffer) {
        return Err(format!(
            "Failed to read contents from source file '{}'.",
            source_path
        )
        .into());
    }
    ...
    if let Err(_) = dest_file.write_all(buffer.as_bytes()) {
        return Err(format!(
            "Failed to write contents to destination file '{}'.",
            dest_path
        )
        .into());
    }
